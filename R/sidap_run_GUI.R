...
#' 
#' This GUI provides an easy way for CyToF data analysis using \code{sidap} package. All parameters 
...
cytof_tsne_densvm_GUI <- function() {
    ...
    reset_para_data <- function() {
        ...
        if (tclvalue(fcsFile) == "" && tclvalue(rawFCSdir) == 
            "") {
            tkmessageBox(title = "SIDAP: Simultaneous Illustration of Diversity And Progression", 
    ...
    transformMethod_help <- function() {
        tkmessageBox(title = "transformMethod", message = "The method used for dimensionality reduction, including \"pca\", \"isomap\", \"tsne\",\"lle\". \n\nIf \"NULL\" was selected, no dimension reduction will be performed.", 
            icon = "info", type = "ok")
    }
    ...
    tkwm.title(tt, "SIDAP: Simultaneous Illustration of Diversity And Progression")
    ...
    tkpack(tkradiobutton(transformMethod_rbuts, text = "lle", 
        variable = transformMethod, value = "lle"), side = "left")
...
getParameters_GUI <- function(fcsFile, rawFCSdir) {
    ...
    markers <- col_names
    markers <- sort(markers[!(is.na(markers))])
    if (length(markers) == 0) {
